...
**Logging:**
- Line 272: `"Running agent {agent} in Docker container for project {project}"`
- Line 309: `"Adding HTTP MCP server: {name} at {url}"`
- Line 317: `"Adding stdio MCP server: {name} with command: {command}"`
- Line 329: `"Using Docker image: {image} for agent {agent}"`
- Line 932: `"Using Claude model in Docker: {model}"`
- Line 943: `"Pre-launch safety check: Verifying container write access..."`
- Line 970: `"Wrote prompt to {path} ({len} chars)"`
- Line 1050: `"Executing detached: docker run -d ..."`
- Line 1140: `"Registered active container: {container_name}"`
- Line 1200: `"Launched detached container: {container_id}"`
- Line 1220: `"Agent completed successfully in container, result length: {len}"`
...
**File:** `services/git_workflow_manager.py:~800-1000`

```python
async def commit(self, message: str):
    # Run git commit with pre-commit hooks
    ...
    if result.returncode != 0:
        # Pre-commit hooks may have failed
...
- `GitWorkflowManager`: `"Committed changes: {message}"`
- `GitWorkflowManager`: `"Pushed branch {branch}"`
- `GitWorkflowManager`: **ERROR**: `"Failed to commit: {stderr}"` (if pre-commit fails)
...
| Log Source | Orchestrator Logs | Container Logs | Redis | Elasticsearch |
|------------|-------------------|----------------|-------|---------------|
| Task queue processing | ✅ | ❌ | ❌ | ✅ (events) |
| Agent executor | ✅ | ❌ | ❌ | ✅ (events) |
| Workspace preparation | ✅ | ❌ | ❌ | ❌ |
| Docker runner | ✅ | ❌ | ❌ | ❌ |
| Container wrapper | ❌ | ✅ (ephemeral) | ✅ | ✅ |
| Claude Code output | ❌ | ✅ (ephemeral) | ✅ | ✅ |
