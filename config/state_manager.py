...
# Re-exported, not defined here. The resolution moved to config/paths.py so
# that callers which must NOT build this module's GitHubStateManager singleton
# (scripts/, services/data_retention.py) can reach it without importing this
# module at all -- see config/paths.py's docstring for the measurement. Every
# existing `from config.state_manager import orchestrator_state_root` and
# every `patch('config.state_manager.orchestrator_state_root')` keeps working
# through this name.
from .paths import orchestrator_state_root  # noqa: F401
