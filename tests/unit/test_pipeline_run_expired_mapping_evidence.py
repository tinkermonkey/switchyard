...
class FakeRedis:
    ...
    def eval(self, script, numkeys, key, field, expected_value):
        """Stand-in for the server running _COMPARE_AND_DELETE_HASH_FIELD_SCRIPT.

        The identity assertion matters: it pins that the reap goes through the
        module's shared compare-and-delete script rather than an ad-hoc
        unconditional HDEL. This fake does NOT certify the Lua text itself --
        see TestReapUsesCompareAndDelete, which asserts on the recorded call.
        """
        assert script is _COMPARE_AND_DELETE_HASH_FIELD_SCRIPT, (
            "expired-mapping cleanup used some other Redis script"
        )
...
class TestStartupSweepStillSeesTheRun:
    ...
    lookup happens (a watchdog pass, a status update, any caller), then the run
    writes its record back -- update_run_status()/_persist_run_update() rewrite
    the record ONLY, never the mapping. If the lookup deleted the mapping entry,
    nothing re-creates it, and get_active_run_workspaces() -- which iterates the
    mapping to find run ids before reading their records -- can no longer see
    the run at all. The startup sweep then prunes a live run's epic worktree.
    """
    ...
    def test_worktree_protection_survives_a_lookup_during_the_expiry_window(self):
        ...
        # The run reaches its next stage and writes its record back -- record
        # only; nothing here re-creates the issue mapping entry.
        manager.update_resolved_workspace(run)
