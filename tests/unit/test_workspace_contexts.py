...
class TestIssuesWorkspaceContext:
    ...
    async def test_issues_workspace_prepares_feature_branch(self, agent_executor):
        ...
        with patch('services.feature_branch_manager.feature_branch_manager') as mock_fbm, \
            ...
            # Verify workspace was resolved via resolve_workspace(), not the
            # old independent prepare_feature_branch() base-clone checkout.
            assert mock_prm.resolve_workspace.called
    ...
    async def test_issues_workspace_finalizes_feature_branch(self):
        ...
        finalize_execution() is untouched by #122 -- only prepare_execution()
        (mocked here via resolve_workspace(), see
        test_issues_workspace_prepares_feature_branch above) changed."""
        from services.agent_executor import AgentExecutor
    ...
    async def test_issues_workspace_uses_git_directory(self):
        """Issues workspace should use the epic worktree directory
        resolve_workspace() resolved onto the pipeline run (#122) -- no
        longer workspace_manager.get_project_dir() directly (the pre-#122
        behavior this replaced, when IssuesWorkspaceContext.
        get_working_directory() was intentionally base-clone-scoped)."""
        from services.agent_executor import AgentExecutor
        ...
        async def fake_resolve_workspace(pipeline_run, github, workspace_type):
            pipeline_run.branch_name = 'feature/test-branch'
            pipeline_run.project_dir = '/workspace/.orchestrator/worktrees/test-project/123'
            return pipeline_run
        ...
        with patch.object(executor, 'factory') as mock_factory, \
            ...
            # Verify the resolved epic worktree, not the shared base clone,
...
class TestWorkspaceContextBehaviorEquivalence:
    ...
    async def test_both_workspaces_execute_agent(self):
        ...
        for workspace_type in ['issues', 'discussions']:
            ...
            if workspace_type == 'discussions':
                ...
                # 'issues' now resolves its branch/worktree via
                # PipelineRunManager.resolve_workspace() (#122) -- needs a
                # pipeline_run_id to look up, mocked below.
                task_context['pipeline_run_id'] = 'run-123'
    ...
    async def test_workspace_type_determines_git_operations(self):
        ...
        prepare_feature_branch() is no longer one of them for 'issues' (#122:
        prepare now goes through resolve_workspace() instead) --
        finalize_feature_branch_work() is unaffected and still the finalize
            ...
            with patch('services.feature_branch_manager.feature_branch_manager') as mock_fbm, \
                ...
                with patch.object(executor, 'factory') as mock_factory, \
                    ...
                    git_operation_count[workspace_type] = {
                        'resolve_workspace': mock_prm.resolve_workspace.call_count,
                        'prepare_feature_branch': mock_fbm.prepare_feature_branch.call_count,
                        'finalize': mock_fbm.finalize_feature_branch_work.call_count,
                    }

        # Verify git operations only happen for issues
        assert git_operation_count['issues']['resolve_workspace'] > 0
        assert git_operation_count['issues']['prepare_feature_branch'] == 0
        assert git_operation_count['issues']['finalize'] == 1
        assert git_operation_count['discussions']['resolve_workspace'] == 0
        assert git_operation_count['discussions']['prepare_feature_branch'] == 0
