...
downstream ever got a chance to refuse. Verifying the other two paths while
leaving this one open would have made #149's fix a half-fix.
"""
...
class TestVerifyFailsafeBranch:
    ...
    def test_accepts_the_dispatchs_own_branch(self, executor):
        with patch('subprocess.run', side_effect=_git_stub()):
            ...
            assert check.commit_branch == 'feature/issue-5-epic'
    ...
    def test_refuses_another_issues_branch(self, executor):
        ...
        with patch('subprocess.run', side_effect=_git_stub(branch_stdout='fix-attempt')):
            ...
            # There WAS a target to drift from, so this blocks the run.
    ...
    def test_refuses_a_detached_head(self, executor):
        ...
        with patch('subprocess.run', side_effect=_git_stub(branch_stdout='HEAD')):
            ...
            assert check.commit_branch is None
            # Logged, not escalated: there was no target to drift from.
            assert check.escalate is False

    def test_a_detached_head_where_a_branch_was_expected_escalates(self, executor):
        with patch('subprocess.run', side_effect=_git_stub(branch_stdout='HEAD')):
            check = executor._verify_failsafe_branch(
                project_dir='/workspace/x',
                project_name='test-project',
    ...
    def test_refuses_the_default_branch_even_without_an_expectation(self, executor, default_branch):
        ...
        Refused but NOT escalated: the shared base clone sitting on its default
        branch is the NORMAL state of a dispatch that never resolved a workspace
        (dev_environment_setup and friends), and blocking the board on it would be
        an over-escalation, not a safety net.
        """
    ...
    @pytest.mark.parametrize('default_branch', ['main', 'master'])
    def test_the_default_branch_where_a_feature_branch_was_expected_escalates(
    ...
    def test_refuses_when_the_branch_cannot_be_read(self, executor):
        """A refusal that carries no branch at all -- which is what tells the
        escalation to render "could not be determined" rather than naming one."""
    ...
    def test_an_unreadable_branch_with_no_expectation_is_not_escalated(self, executor):
        with patch('subprocess.run', side_effect=_git_stub(branch_rc=128)):
            check = executor._verify_failsafe_branch(
                project_dir='/workspace/x',
                project_name='test-project',
...
class TestFailsafeCommitCheckHonorsTheExpectation:
    ...
    async def test_a_wrong_branch_stages_nothing(self, executor):
        with patch('subprocess.run', side_effect=_git_stub(
            ...
            assert refusal.escalate is True
            assert refusal.current_branch == 'fix-attempt'
    ...
    async def test_the_matching_branch_commits_as_before(self, executor):
        ...
        with patch('subprocess.run', side_effect=_git_stub(status_stdout='MM app.py')), \
             patch.object(executor, '_failsafe_stage_and_commit',
                          new_callable=AsyncMock) as mock_commit:
    ...
    @pytest.mark.asyncio
    async def test_a_clean_workspace_is_not_a_refusal(self, executor):
