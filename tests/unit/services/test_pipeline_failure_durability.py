...
class TestCreateLockWriteVerification(unittest.TestCase):
    ...
    try_acquire_lock()'s YAML-fallback path relies on to know whether a lock
    write actually landed anywhere. (Historically steal_lock() was the other
    caller depending on this too, before #58/Phase 2 of #34's concurrency
    redesign removed it outright — repair cycles now acquire via plain
    try_acquire_lock(), the same primitive as everything else, with no
    eviction.) Before this fix, _create_lock() had no return value at all: a
    Redis failure was only logged, and the YAML write's own success/failure
    return from _save_lock_to_yaml() was silently discarded — so a caller
    could report success ("lock_acquired") even when neither store actually
    persisted anything."""
