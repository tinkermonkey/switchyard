            ...
            <table className="w-full">
              <thead className="border-b border-gh-border">
                <tr>
                  ...
                  <SortHeader field="avg_max_context" label="Avg Peak Context" />
                                  ...
                                  <table className="w-full text-xs">
                                    <thead>
                                      <tr className="text-gh-fg-muted">
                                        ...
                                        <th className="text-right font-normal pb-1">Avg Peak Ctx</th>
                                      </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gh-border">
                                      {models.map(mod => {
        ...
        <div className="space-y-4 text-xs text-gh-fg-muted">
          <div>
            ...
            <table className="w-full">
              <tbody className="divide-y divide-gh-border">
                {[
                  ['Avg Direct Input', 'Average non-cached input tokens billed at full rate, summed across all turns per execution. For multi-turn agents this accumulates with each API call and can exceed the initial context size.'],
                  ...
                  ['Avg Peak Context', 'Average of the peak context window size reached during each execution. Context accumulates as tool results are appended; a high value may indicate long-running or tool-heavy tasks. The range shows min and max across executions.'],
                ].map(([col, def]) => (
                  <tr key={col}>
            ...
            <table className="w-full">
              <tbody className="divide-y divide-gh-border">
                {[
                  ...
                  ['Avg Peak Ctx', 'Average peak context window size per task when this model was used.'],
                ].map(([col, def]) => (
                  <tr key={col}>
                    <td className="py-1.5 pr-4 font-mono text-gh-fg whitespace-nowrap align-top w-40">{col}</td>
                    <td className="py-1.5">{def}</td>
