        ...
        <div className="space-y-4 text-xs text-gh-fg-muted">
          <div>
            ...
            <table className="w-full">
              <tbody className="divide-y divide-gh-border">
                {[
                  ['Avg Direct Input', 'Average tokens sent in the prompt that are not cache hits, per execution of this cycle type. Billed at full input rate.'],
                  ...
                  ['Avg Max Context', 'Average of the peak context window size reached during each execution. Context grows as tool results are appended; a high value indicates long-running or tool-heavy cycles. The range shows min and max across executions.'],
                  ['Avg Initial Context', 'Average context window size at the very start of each execution, before any tool calls. Reflects the size of the initial prompt passed into the cycle (system prompt + task description + any pre-loaded history).'],
                ].map(([col, def]) => (
                  <tr key={col}>
