"""
Claude Investigation Orchestrator

Orchestrates investigation lifecycle for Claude Code tool execution failures.
"""

import logging
from typing import Dict, Any, Tuple
import redis
from elasticsearch import Elasticsearch

from services.medic.base import BaseInvestigationOrchestrator
from services.medic.claude.claude_signature_store import ClaudeFailureSignatureStore
from services.medic.claude.claude_investigation_queue import ClaudeInvestigationQueue
from services.medic.claude.claude_agent_runner import ClaudeInvestigationAgentRunner
from services.medic.claude.claude_report_manager import ClaudeReportManager

logger = logging.getLogger(__name__)


class ClaudeInvestigationOrchestrator(BaseInvestigationOrchestrator):
    """
    Claude-specific investigation orchestrator.

    Manages investigation lifecycle for Claude Code tool execution failures.
    """

    def __init__(
        self,
        redis_client: redis.Redis,
        es_client: Elasticsearch,
        workspace_root: str = "/workspace/clauditoreum",
        medic_dir: str = "/medic",
    ):
        """
        Initialize Claude investigation orchestrator.

        Args:
            redis_client: Redis client for queue management
            es_client: Elasticsearch client for signature data
            workspace_root: Path to Clauditoreum codebase
            medic_dir: Base directory for investigation reports
        """
        # Initialize Claude-specific components
        queue = ClaudeInvestigationQueue(redis_client)
        agent_runner = ClaudeInvestigationAgentRunner(workspace_root)
        report_manager = ClaudeReportManager(medic_dir)
        failure_store = ClaudeFailureSignatureStore(es_client)

        # Initialize base orchestrator
        super().__init__(
            redis_client=redis_client,
            es_client=es_client,
            queue=queue,
            agent_runner=agent_runner,
            report_manager=report_manager,
            failure_store=failure_store,
            workspace_root=workspace_root,
            medic_dir=medic_dir,
        )
        logger.info("ClaudeInvestigationOrchestrator initialized")

    def _prepare_investigation_context(
        self, fingerprint_id: str, signature: Dict[str, Any]
    ) -> Tuple[str, str]:
        """
        Prepare investigation context file and output path for Claude failures.

        Args:
            fingerprint_id: Fingerprint ID
            signature: Signature document from Elasticsearch

        Returns:
            Tuple of (context_file_path, output_log_path)
        """
        # Get project and sample clusters
        project = signature.get("project", "unknown")
        sample_clusters = signature.get("sample_entries", signature.get("sample_clusters", []))[:10]

        # Write context file
        context_file = self.report_manager.write_context(
            fingerprint_id=fingerprint_id,
            signature_data=signature,
            sample_clusters=sample_clusters,
            project=project,
        )

        # Get output log path
        output_log = self.report_manager.get_investigation_log_path(fingerprint_id)

        return context_file, output_log

    def _validate_investigation_result(self, fingerprint_id: str) -> Tuple[str, str]:
        """
        Validate investigation result based on report files for Claude failures.

        Args:
            fingerprint_id: Fingerprint ID

        Returns:
            Tuple of (result, status) where:
            - result: "success", "ignored", "failed", "timeout"
            - status: "completed", "ignored", "failed", "timeout"
        """
        # Use the Claude-specific get_status method
        status = self.report_manager.get_status(fingerprint_id)

        if status == "diagnosed":
            return self.queue.RESULT_SUCCESS, self.queue.STATUS_COMPLETED
        elif status == "ignored":
            return self.queue.RESULT_IGNORED, self.queue.STATUS_IGNORED
        else:
            return self.queue.RESULT_FAILED, self.queue.STATUS_FAILED

    def _get_observability_data(
        self, fingerprint_id: str, signature: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        Get observability event data for Claude investigation.

        Args:
            fingerprint_id: Fingerprint ID
            signature: Signature document

        Returns:
            Dict with observability event data
        """
        return {
            "fingerprint_id": fingerprint_id,
            "type": "claude",
            "project": signature.get("project", "unknown"),
            "severity": signature.get("severity"),
            "error_type": signature.get("signature", {}).get("error_type"),
            "tool_name": signature.get("signature", {}).get("tool_name"),
        }
