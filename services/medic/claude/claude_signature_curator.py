...
class ClaudeSignatureCurator:
    def __init__(self):
        ...
        self.old_store = DockerFailureSignatureStore(self.es_client)  # For cleaning up old system data
    ...
    async def run_curation_cycle(self):
        ...
        try:
            ...
            # Delete Claude system signatures from Elasticsearch (returns count and list of deleted IDs)
            ...
            if deleted_fp_ids:
                ...
                logger.info(f"Claude system cleanup complete: {deleted_count} signatures removed, "
                           f"{reports_deleted} report directories deleted, "
                           f"{redis_cleaned} Redis keys cleaned")
            else:
                logger.info("No stale Claude signatures to clean up")

            # Also cleanup old system signatures
            old_deleted_count, old_deleted_fp_ids = self.old_store.cleanup_stale_signatures(retention_days)
            if old_deleted_count > 0:
                logger.info(f"Old system cleanup complete: {old_deleted_count} signatures removed")
            else:
                logger.info("No stale old system signatures to clean up")
