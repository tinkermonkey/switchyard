...
class FeedbackManager:
    ...
    def mark_comment_processed(self, issue_number: int, comment_id: str, project: str = None) -> bool:
        """Mark a comment as processed to prevent re-processing"""
        try:
            key = f"feedback:issue:{issue_number}:processed_comments"
            self.redis.sadd(key, comment_id)
            self.redis.expire(key, self.default_ttl)

            logger.debug(f"Marked comment {comment_id} as processed for issue #{issue_number}")
            return True
        except Exception as e:
            logger.error(f"Failed to mark comment as processed: {e}")
            return False

    def is_comment_processed(self, issue_number: int, comment_id: str) -> bool:
    ...
    def set_last_agent_comment_time(self, issue_number: int, agent: str, timestamp: str):
        """Record when an agent last commented on an issue"""
        try:
            key = f"feedback:issue:{issue_number}:last_agent_comment:{agent}"
            self.redis.set(key, timestamp)
            self.redis.expire(key, self.default_ttl)

            logger.debug(f"Recorded last comment time for {agent} on issue #{issue_number}: {timestamp}")
        except Exception as e:
            logger.error(f"Failed to set last agent comment time: {e}")

    def get_last_agent_comment_time(self, issue_number: int, agent: str) -> Optional[str]:
    ...
    def store_feedback_context(self, issue_number: int, agent: str, context: Dict[str, Any]):
        """Store feedback context for an issue"""
        try:
            import json
            key = f"feedback:issue:{issue_number}:context:{agent}"
            self.redis.set(key, json.dumps(context))
            self.redis.expire(key, self.default_ttl)

            logger.debug(f"Stored feedback context for {agent} on issue #{issue_number}")
        except Exception as e:
            logger.error(f"Failed to store feedback context: {e}")

    def get_feedback_context(self, issue_number: int, agent: str) -> Optional[Dict[str, Any]]:
    ...
    def clear_issue_feedback(self, issue_number: int):
        """Clear all feedback tracking for an issue (useful when issue is closed)"""
        try:
            pattern = f"feedback:issue:{issue_number}:*"
            keys = self.redis.keys(pattern)
            if keys:
                self.redis.delete(*keys)
                logger.info(f"Cleared feedback tracking for issue #{issue_number}")
        except Exception as e:
            logger.error(f"Failed to clear issue feedback: {e}")

    def get_processed_comment_count(self, issue_number: int) -> int:
