...
#
# Board polling is this system's largest and least urgent GraphQL consumer:
# ~2,000-2,400 board syncs an hour across ~30 boards, against a 5,000-point
# hourly budget, most of them reporting no change. Pipeline-critical calls --
...
# state 30 seconds later for free, while the card move it starved stopped a
...
# for a large part of every hour. 10% of 5,000 is ~500 points held back, which
# is far more than a stage transition needs and small enough that the loop
# still polls normally in the ordinary case.
#
...
def card_move_retry_delay(
    ...
    """How long _move_card_with_retry() should wait before its next attempt,
    or None when waiting cannot help and it should stop retrying now.

    Two regimes, because the two failures are nothing alike:

    - No rate limit in play (rate_limit_reset_seconds is None or already
      elapsed): plain exponential backoff off `base_delay` -- 5s, 10s, 20s.
      This is the pre-existing behaviour and it is right for a transient
      GitHub blip.

    - The GraphQL quota is exhausted: back off to the QUOTA WINDOW, not to a
      few seconds. This is the confirmed production failure (pipeline run
      4cf816cf). A review cycle approved issue #1045, the card move to
      "Testing" failed three times in 16 seconds -- 5s and 10s apart -- and
      every one of those attempts landed inside the same rate-limit window,
      which still had ~13 minutes left to run. The retries could not have
      succeeded. The board's lock was then durably retained and the pipeline
      stopped until a human intervened, for a condition that would have
      cleared on its own.

    Returns None (stop now) when the reset is further out than
    max_rate_limit_wait: parking a lock-holding daemon thread for most of an
    hour is its own failure mode, and the caller's exhausted-retries path now
    ends the run properly rather than leaving it open (see
    review_cycle_thread_teardown's card_move_failed).
    ...
    return rate_limit_reset_seconds + 5.0


def github_rate_limit_reset_seconds() -> Optional[float]:
    """Seconds until the shared GitHub breaker's rate-limit window resets, or
    None when no rate limit is in force (or the answer is unavailable).

    Reads the breaker rather than a bucket because the breaker's reset_time is
    set from GitHub's own x-ratelimit-reset header at the moment the limit was
    hit -- it is the authoritative "when can this possibly work again".
    ...
    try:
        ...
        if not (breaker.is_open() or breaker.is_half_open()):
            return None
        if not breaker.reset_time:
            return None
        return max(0.0, (breaker.reset_time - datetime.now()).total_seconds())
    except Exception as e:  # pragma: no cover - defensive
        logger.debug(f"Could not read the GitHub rate-limit reset time: {e}")
        return None
...
class ProjectMonitor:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    ...
                    f"skipping this PR ready check (the periodic orphaned-parent sweep "
                    f"will retry): {e}"
                )
                return
    ...
    def _move_card_with_retry(
        ...
        quota window when the failure is rate limiting and exponentially
        otherwise — see card_move_retry_delay() for why a fixed 5s/10s ladder
        was actively harmful here. On final failure, emits an error decision
        event and posts a GitHub comment. Re-raises CancellationError
        immediately per the cancellation contract.
        ...
        for attempt in range(1, max_retries + 1):
            ...
            if attempt < max_retries:
                ...
                wait_time = card_move_retry_delay(attempt, reset_seconds)
                if wait_time is None:
                    logger.error(
                        f"Not retrying the card move for issue #{issue_number}: the "
                        f"GitHub rate limit does not reset for another "
                        f"{reset_seconds:.0f}s, which is longer than this path will "
                        f"wait ({CARD_MOVE_MAX_RATE_LIMIT_WAIT_SECONDS:.0f}s). Every "
                        f"further attempt inside that window would fail identically."
                    )
                    break
                if reset_seconds:
                    logger.warning(
    ...
    def _start_pr_review_for_issue(
        ...
        Args:
            phantom_run_id: If the caller already created a pipeline run for this
                ...
                on every early-return path that fires before that call (the
                review-cycle-limit guard, the duplicate-execution guard, the
                lock-acquire failure) so the phantom never lingers as a permanently
                "active" run. Once get_or_create_pipeline_run() resolves it,
                `_owned_run_id` is reassigned to the real run's id and
                `_owned_is_real_run` is set — so if setup fails after that point but
                before the background thread starts, the outer except instead routes
                through `_end_pr_review_pipeline_run_on_failure()` (the same
                failure-handling logic an in-thread failure would use), not
                `_end_owned_run_if_pending()`, so a genuine failure gets a proper
                outcome="failed" and retain-vs-release lock decision rather than
                being stamped outcome="superseded" like an actually-superseded
                phantom. The background thread itself owns cleanup for anything
                that fails after it starts (see run_pr_review() below).
        """
        ...
        # `_owned_is_real_run` distinguishes "still just the original phantom" from
        # "now the real run this method is driving" — see its use in the outer
        # except block below.
        _owned_run_id = phantom_run_id
        _settled = False
        _owned_is_real_run = False
        ...
        try:
            ...
            if _owned_is_real_run and _owned_run_id and not _settled:
                # This is the real pipeline run (not the original phantom) failing
                # during setup, before the background thread — which would normally
                # own failure handling via _end_pr_review_pipeline_run_on_failure —
                # ever started. Route through that same function rather than
                # _end_owned_run_if_pending()/end_phantom_pipeline_run(), which
                # unconditionally stamps status="completed"/outcome="superseded":
                # a genuine failure here should get the same NonRetryableAgentError-
                # aware retain-vs-release lock decision and durable failure marking
                # (mark_failed()) an in-thread failure would. That function already
                # makes the correct call on the lock in both its branches, so the
                # ad-hoc lock release below must be skipped in this case — doing it
                # unconditionally would silently undo a durable retain-on-failure.
                _settled = True
                try:
                    _end_pr_review_pipeline_run_on_failure(
                        self.pipeline_run_manager, project_name, board_name, issue_number, e,
                    )
                except Exception as end_err:
                    logger.warning(f"Failed to end pipeline run after PR review setup failure: {end_err}")
                return None

            # End whichever run this method is still responsible for (the original
            # phantom if setup failed before get_or_create_pipeline_run()).
    ...
    def _start_repair_cycle_for_issue(
        ...
        Args:
            phantom_run_id: See _start_pr_review_for_issue's docstring for the same
                parameter — the caller's eagerly-created run. Tracked via
        ...
        # its first use: Python's function-scoping rule makes `work_execution_
        # tracker` a local name for this method's ENTIRE body the moment it's
        # imported ANYWHERE inside it, so the outer except block's own `'work_
        # execution_tracker' in dir()` guard only sees it as available once
        # this import has actually EXECUTED -- an exception raised before
        # whichever local import used to run first (e.g. epic-worktree
        # creation failing, well before the record_execution_start() call this
        # was previously only imported alongside) would silently skip
        # recording the failure entirely, breaking the retry/threshold logic
        # in trigger_agent_for_status (this method's caller) that depends on
        # it. Importing it unconditionally up front means every exception
        # path below can rely on it being present.
        from services.work_execution_state import work_execution_tracker
        ...
        try:
                ...
                try:
                    ...
                    if existing_container_name:
                        ...
                        if result.stdout.strip():
                            ...
                            _end_owned_run_if_pending("Duplicate repair cycle container already running - ending phantom run")
            ...
            if not test_configs:
                ...
                _end_owned_run_if_pending("No test configurations found - ending phantom run")
                return None
            ...
            # Defensive cleanup: if the run above did NOT turn out to be the phantom
            # run, end the phantom explicitly — same rationale as
            # _start_pr_review_for_issue's equivalent check.
            if _owned_run_id and _owned_run_id != pipeline_run.id:
                ...
                if _owned_is_real_run and _owned_run_id and not _settled:
                    ...
                    try:
                        self.pipeline_run_manager.end_pipeline_run(
                            ...
                            issue_number=issue_number,
                            board=board_name,
                            reason=f"Failed to save repair cycle context: {e}",
                        )
                    except Exception as cleanup_e:
                        logger.error(f"Failed to end pipeline run after context-save failure: {cleanup_e}")
                return None
...
"""
            ...
            if not container_name:
                ...
                try:
                    ...
                    if not ended:
                        logger.warning(
                            f"end_pipeline_run found no active run to end for {project_name} "
                            f"issue #{issue_number} after container launch failure"
                        )
                except Exception as cleanup_e:
            ...
            import traceback
            logger.error(traceback.format_exc())
            # Record the failure, but do NOT end the pipeline run / release the
            # pipeline lock here. Found live in production: a repeated collision
            # between this function's epic-worktree creation and ordinary
            # dispatch's shared-branch checkout (see _add_epic_worktree's own
            # fix for the git-level side of this) was releasing the lock on
            # every failed attempt, opening a window for a SIBLING sub-issue of
            # the same epic to dispatch in between retries -- exactly the
            # scenario the epic-worktree model assumes can't happen (an epic's
            # sub-issues are meant to be single-threaded). Leaving the run/lock
            # held instead means the issue keeps its exclusive claim across
            # every retry, so no sibling can ever race in.
            #
            # This isn't a silent hang: trigger_agent_for_status (this method's
            # caller) already has the exact retry/threshold logic needed for an
            # issue that holds the lock but whose last execution for this
            ...
            if 'pipeline_run' not in dir():
                ...
                # nothing to hold for a retry. `_owned_run_id` is still whatever
                # phantom_run_id the caller passed (or None); clean up just that
                # phantom bookkeeping, unrelated to the board's pipeline lock.
                try:
                    _end_owned_run_if_pending(f"Repair cycle startup error before pipeline run resolved: {e}")
                except Exception as cleanup_e:
    ...
    def monitor_projects(self):
        ...
        while True:
            try:
                ...
                # board fetching: it is a throttle on polling, not on the
                # orchestrator. Agent dispatch, review cycles and card moves run
                # on worker threads that this pause does not touch, which is the
                # whole point -- the budget the monitor stops spending here is
                # the budget those paths get to spend instead.
                    ...
                    if not hasattr(self, '_last_budget_backoff_warning') or \
                       (now - self._last_budget_backoff_warning).total_seconds() >= 60:
                        logger.warning(
                            ...
                            f"{budget_backoff:.0f}s — agent dispatch, review cycles "
                            f"and card moves are unaffected."
                        )
